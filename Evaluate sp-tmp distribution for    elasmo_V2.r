...
#
# 
# Original script created by Loes, Gudjon, Giuseppe & Vaishav (25/03/2020; updated on 26/03/2022 by Loes)
...
# Beam width is the number stated in Gear, so we extract that number into a new variable 
dat2 <- dat2 %>% 
...
datspec<-dat %>% 
  mutate(scientificname = if_else(scientificname=="Mustelus asterias", "Mustelus", scientificname))%>%mutate(scientificname = if_else(scientificname=="Mustelus mustelus", "Mustelus", scientificname))%>%mutate(Valid_Aphia = if_else(Valid_Aphia==105821, 105732, Valid_Aphia))%>%mutate(Valid_Aphia = if_else(Valid_Aphia==105822, 105732, Valid_Aphia)) %>%filter(Valid_Aphia %in% c(105883,105891,367297,105885,105887,105865,105876,105885,105820,105732,105814,105815,105923))
...
jpeg(file=paste("rayas_elasmobranch_by_source.jpeg",sep=""),width = 200, height = 200, units = "mm", res = 400)
...
jpeg(file=paste("sharks_elasmobranch_by_source.jpeg",sep=""),width = 200, height = 200, units = "mm", res = 400)
ggplot(number_measured %>% filter(Year %in% c(2000:2023), scientificname %in% sharks))+
  geom_bar(aes(x=Year, y=NumMeas, fill=Country), stat = "identity", position="dodge") +
  facet_wrap(~scientificname, scales="free_y")
dev.off()
...
jpeg(file=paste("rayas_elasmobranch_totN.jpeg",sep=""),width = 200, height = 200, units = "mm", res = 400)
...
ggplot()+
  ...
  coord_quickmap(xlim = c(xmin, xmax), ylim = c(ymin, ymax)) +theme(text=element_text(size=22))+
  theme(legend.title = element_text(face = "bold", size = 18))+ theme(legend.position=c(0.75,0.2),legend.direction = "horizontal", axis.title = element_text(size = 18))+ theme(legend.text=element_text(face = "italic",size=16))+ guides(shape = guide_legend(nrow=4, override.aes = list(size = 8)))+ scale_shape_manual(values=c(15,8,17,16,18,10,7))
dev.off()  
...
jpeg(file=paste("rays_map_TOT.jpeg",sep=""),width = 300, height = 400, units = "mm", res = 300)
  ggplot()+
  theme_light()+
  geom_polygon(data=m,aes(long,lat,group=group),fill=NA,color="grey") +  #plot the land
  ...
  labs(x = "Longitude (Degrees)", y = "Latitude (Degrees)", title = "Rajas CPUE by haul", fill="CPUE (sqrt n/km^2)") +
  coord_quickmap(xlim = c(xmin, xmax), ylim = c(ymin, ymax)) + theme(text=element_text(size=22))+
  theme(legend.title = element_text(face = "bold", size = 18)) + theme(legend.position="bottom", axis.title = element_text(size = 18))+ theme(legend.text=element_text(face="italic",size=16))+ guides(shape = guide_legend(nrow=4, override.aes = list(size = 8)))
dev.off()
...
ggplot()+
  ...
  coord_quickmap(xlim = c(xmin, xmax), ylim = c(ymin, ymax)) +theme(text=element_text(size=22))+
  theme(legend.title = element_text(face = "bold", size = 18))+ theme(legend.position=c(0.88,0.2), axis.title = element_text(size = 18))+ theme(legend.text=element_text(face = "italic",size=16))+ guides(shape = guide_legend (override.aes = list(size = 8)))
dev.off()
